...
Vitro 已有 300+ 测试用例，但 C 语言极其灵活，即使如此规模的测试，面对真实世界的 C 代码仍可能暴露未发现的边缘问题。更关键的是：
...
│  → 直接由 vitro_vm::core::executor 原生执行，Rust 实现，带边界检查        │
├─────────────────────────────────────────────────────────────┤
...
- 这是**编译器自举的轻量版**。如果 Vitro 能正确编译并执行自己的 C 代码，说明编译器和 VM 的语义实现是自洽的。
...
| 缺口 | 影响 | 现状 |
|---|---|---|
| **3. Bytecode Libc 产品化** | 学生代码调用 `isdigit(c)` 时走的是 **Rust Host Func**，不是 Bytecode Libc 的 C 实现；无法展示"libc 源码"教学价值 | ✅ **已完成（2026-06-07）**。构建期预编译脚本 `scripts/precompile_bytecode_libc.py` + `vitro_cli export` 已建立；全局函数表固定索引段（1000~）已实现；ctype 纯计算函数（`isdigit`/`isalpha`/.../`abs`）已切换为 Bytecode 路径；**2026-06-07 追加：`strlen`/`strcmp` 已加入 Bytecode Libc 产品路径**；`bytecode_libc_consistency.rs` 和 `differential_stress.rs` 测试验证通过 |
...
Round 3（P1 架构）：✅ 已完成（2026-06-07）
  ├─ ✅ 构建期预编译脚本 `scripts/precompile_bytecode_libc.py` + `vitro_cli export`
