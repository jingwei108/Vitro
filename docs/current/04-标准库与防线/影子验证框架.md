...
> 最后核对：2026-09-13（D5 语言迁移最后一站：主驱动 Python→Go）
>
...
> 当前规模：C 侧 **663 个用例**（完全匹配 644 + cide_better 16 + known_issue 3，0 非预期差异），
> C++ 侧 **97 个用例**（95 一致 + 2 个已记录的 `clang_compile_fail`）。
...
| 选项 | 说明 |
|:---|:---|
| `--jobs N` | Clang 并发数；`0` = 自动 min(CPU,16)（Cide 侧始终互斥串行） |
...
Go 驱动 663 用例实测：冷启动全量重算 ~26s（16 路并发）、缓存热跑 ~5s。
...
5. **模板用例生成链断裂（2026-09-11 起）**：`native/tests/cases_template_generated/`（83 个用例）当前只有读取方
   （`native/tests/cide_e2e.rs`、`scripts/shadow_verify.go`），生成器脚本 `scripts/sync_templates.py`
