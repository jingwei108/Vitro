...
| 脚本 | 功能 |
|:---|:---|
| [`shadow_verify_cpp.go`](../../../scripts/shadow_verify_cpp.go | C++ Shadow Verification 驱动（与 Clang++ 对照；D5 Go 迁移版，Clang 并发 16 路）。Python 版已在 D5 收官时退役删除 |
| `scripts/serve_smoke`（Go） | `vitro_cli serve` JSON-lines 协议冒烟（57 项断言，自报口径；go run ./scripts/serve_smoke） |
| `scripts/ci_three_tier_check`（Go） | CI 三层一致性检查（失败记录 ↔ 测试结果双向对账；`go run ./scripts/ci_three_tier_check`） |
| `scripts/engineering_health`（Go） | 工程健康度看板（`go run ./scripts/engineering_health`） |
| `scripts/precompile_bytecode_libc`（Go） | Bytecode Libc 构建期预编译（生成固定索引段数据；`go run ./scripts/precompile_bytecode_libc`） |
