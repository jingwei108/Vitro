...
> ABI 版本通过 `cide_abi_version()` 查询（当前 `1.2.0`）。
...
- `ensure_abi()`（D5 迁移后位于 `scripts/internal/capi`，C/C++ 两侧 Go 驱动共用）除校验 ABI 符号外，还比对 `cide_engine_version()` 与 `git rev-parse --short HEAD`，不一致即抛错（原 Python 伴生模块 `cide_output.py` 已随驱动 Go 化退役）；
...
覆盖 id 关联 / 帧同构 / 会话生命周期 / 配置一致性 / 三段式内存地图 / schema 轨道与词汇表的 51 项断言（CI 已纳入，脚本自报口径）。
...
| [`serve_smoke.py`](../../../scripts/serve_smoke.py | `cide_cli serve` JSON-lines 协议冒烟（51 项断言，自报口径） |
