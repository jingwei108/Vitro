...
| 出口 1 · C ABI | ✅ 可用；第一批 13 个入口全部落地，`cide_abi_version()` = 1.2.0 |
...
| 测试防线 | ✅ 五层防线全绿：C Shadow 671 用例、C++ Shadow 97 用例、`cargo test` 925 passed / 0 failed、clippy 0 warning |
...
| `cide_cli serve`（id 关联 / 帧同构 / `session.reset`） | ✅ 落地并进 CI 冒烟（51 项断言） |
