...
> **v3 重提取与 golden 固化（2026-09-14，防线 6① 收官批）**：二审全部 P0/P1
> 修复（含管道批 P0-4/P1-3/P1-4 收口）落地后的全量重提取完成——**82 模板 =
> 37 有标注（317 条首现 / 113 个 (模板,phase) 行）+ 45 零标注**（比二审 §4.2
> 预期的 34 多出的 3 个 = P0-B 批复亮的 bstInsert/bstSearch/bstDelete，口径吻合）。
> 行为基线已固化为 **`native/tests/golden/algorithm_annotations_v3.json`** 并接入
> CI（`algorithm_annotation_golden_test`：Rust 直调 session_api 全量比对，
> 双向防漂移——新复亮/新转零/首现序列/挂载行任一变化即红；J9 埋雷已证红）。
> **golden 定位 = 行为基线快照，非语义认证**——本清单 §2 主表的人审勾选继续
> 在此基线上进行（勾选结论更新 golden 须附红→绿锚）；重提取原始数据：
> `tmp/annot_extract_v3_20260914.json`（37 模板逐条含 vars/seq）。
> **登记**：① 结束后 step.next 重复发布末帧（run_batch 终态重放，违反"每真实步
> 恰投递一次"——消费方 finished 即停故低危，待修）；② 二审 §6 判据层专属锚
> 未单独补齐，但 golden 全量锚（317 条）已实质覆盖其防漂移诉求。
...
1. **P0-1 首帧旧值**（用户实测三例）：同一语句的多帧中首帧在赋值发生前，
   ...
   **实测三例首帧错误值全部消失**（2026-09-13 复审后补测：`binary` 的
   ...
   > （宁可少报一条，不可把赋值前旧值当正确值呈现）。`serve_smoke` 57 PASS
