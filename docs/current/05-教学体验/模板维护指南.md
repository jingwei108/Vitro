...
native/tests/cases_template_generated/<key>_default.c     82 个 .c（可重新生成）
        ↓
├─ 防线 1 Shadow Verification：用例目录由 scripts/shadow_verify.go 的 loadCaseFiles() 扫描
│    （src_dir = "template"，Golden 来自 Clang 实时输出）
│    go run ./scripts/shadow_verify
└─ Rust E2E：native/tests/vitro_e2e.rs 的模板用例，断言
...
- 读取 `native/tests/cases_template_generated/` 的消费者：`native/tests/vitro_e2e.rs`、`scripts/shadow_verify.go`（2026-09-13 D5 迁移后；原 Python 驱动 `shadow_verify.py` 与其配套的 `extract_shadow_cases.py` 已退役删除）。
...
5. 在 `native/` 目录下跑 `cargo test --test vitro_e2e`，并跑一次防线 1（见 §5.3），确认没有新增差异。
6. 若确有不一致必须记为已知失败，同步 `native/tests/vitro_e2e.rs` 的 `KNOWN_TEMPLATE_FAILURES` 常量与 `native/tests/E2E_FAILURES.md`（CI 双向对账要求两者一致）。
...
| 出口 | 验证方式 |
|------|---------|
| C ABI / `vitro_cli` | `vitro_cli run native/tests/cases_template_generated/<key>_default.c`（或 `step` / `unified`）与 `cases_golden/<key>_default.out` 比对 |
| wasm32 | 由社区前端接入后自行冒烟；本仓库不含 wasm 侧用例驱动 |
| `vitro_cli serve` | JSON-lines 会话帧冒烟：`python scripts/serve_smoke.py` |

原 Dart 测试覆盖的语义点（模板加载、`ext` 字段、占位符替换、教程模式下切 `main.c`/`main.cpp`）现在属于**社区前端职责**，本仓库不再提供对应测试。

### 5.3 Shadow Verification（防线 1）与 Rust E2E

模板生成的 `native/tests/cases_template_generated/<key>_default.c` 会被纳入 Shadow Verification，Golden 来自 Clang 实时输出（不能来自 Vitro 自己）。

- 驱动：`go run ./scripts/shadow_verify`（`--jobs N` Clang 并发数，`--refresh-clang` 强制全量重算，`--rebuild` 在 release DLL 陈旧时重建）
- Rust E2E：`native/tests/vitro_e2e.rs` 以 `native/tests/cases_golden/<key>_default.out` 为锁定 Golden 做断言
- 已知失败必须双向登记：`KNOWN_TEMPLATE_FAILURES`（`vitro_e2e.rs`）与 `native/tests/E2E_FAILURES.md`，任一防线转绿需同步移除（CI 硬检查）
