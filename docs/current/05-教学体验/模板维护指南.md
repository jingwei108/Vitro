...
- 读取 `native/tests/cases_template_generated/` 的消费者：`native/tests/cide_e2e.rs`、`scripts/shadow_verify.go`（2026-09-13 D5 迁移后；原 Python 驱动 `shadow_verify.py` 与其配套的 `extract_shadow_cases.py` 已退役删除）。
...
- 驱动：`go run scripts/shadow_verify.go`（`--jobs N` Clang 并发数，`--refresh-clang` 强制全量重算，`--rebuild` 在 release DLL 陈旧时重建）
