...
- **Baseline**：`native/tests/cases/baseline/`（360 个，全绿（j1 为双侧编译失败红锚，E2E 登记跳过"必须可运行"契约）；2026-09-06 新增 `codegen_soundness_regression.c` 固化第三批 soundness 修复；2026-09-11 新增 `engine_note_lookalike.c` 固化 E-P1-5 输出通道口径、`scanf_return_value.c` / `scanf_literal_match.c` / `scanf_literal_mismatch.c` 固化 scanf 返回值与普通字符指令）
