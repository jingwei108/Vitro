...
impl Parser {
    ...
    pub(crate) fn parse_template_instantiation(&mut self) -> TemplateInstantiation {
        ...
        while !self.check(TokenType::Gt) && !self.is_at_end() {
            let arg = if self.is_type_token() {
                vitro_ast::TemplateArg::Type(self.parse_base_type())
            } else {
                vitro_ast::TemplateArg::Expr(self.parse_template_arg_expr())
            };
            args.push(arg);
    ...
    pub(crate) fn parse_typedef_enum_decl(&mut self, program: &mut ProgramNode) {
        ...
        if self.match_token(TokenType::Colon) {
            ...
            if !matches!(
                underlying.kind(),
                vitro_ast::TypeKind::Int | vitro_ast::TypeKind::Char | vitro_ast::TypeKind::LongLong
            ) {
                self.errors.push(ParseError {
