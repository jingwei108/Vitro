...
impl TypeChecker {
            ...
            Stmt::RangeFor { var, var_type, iter, body, loc } => {
                ...
                let elem_type = if iter_type.is_array() {
                    ...
                    match iter_type.name() {
                        "vitro_vec_int" | "vitro_list_int" => Type::int(),
                        "vitro_vec_float" => Type::float(),
                        "vitro_vec_char" | "vitro_string" => Type::char(),
                        _ => {
    ...
    pub(crate) fn check_user_func(&mut self, name: &str, args: &mut Vec<Expr>, loc: &SourceLoc) -> Type {
        ...
        if let Some((ret_ty, param_types)) = vitro_runtime::bytecode_libc_sig::bytecode_libc_sig(name) {
